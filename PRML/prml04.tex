...
\begin{frame}
    ...
    %Section 8.6
    これらの比較は1.5.4で詳しく扱った\footnote{\cite{murphy2012}の8.6も詳しい．}．
\end{frame}
...
\begin{frame}
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.5]
            \draw[thick,red] (-2,-2) -- (6,6);
            \draw[thick,red] (1,6) -- (7,-3);
            \node at (3,5) {$?$};
            \node at (0.5,3) {$\mathcal{R}_1$};
            \node at (2.8,1) {$\mathcal{R}_3$};
            \node at (4.7,2.7) {$\mathcal{R}_2$};
            \draw[<->,thick] (-1,1) node[anchor=south] {$\mathcal{C}_1$}
            -- (1,-1) node[anchor=north] {$\mathrm{not}\ \mathcal{C}_1$};
            \draw[<->,thick] (3.8,-1) node[anchor=north] {$\mathrm{not}\ \mathcal{C}_2$}
            -- (6.2,0.6) node[anchor=south] {$\mathcal{C}_2$};
        \end{tikzpicture}
    \end{center}
\end{frame}
...
\begin{frame}
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.5]
            \def\x{-2}
            \def\y{2.95}
            \draw[thick,red] (\x,\x) -- (\y,\y);
            \draw[thick,dotted,red] (\y,\y) -- (6,6);
            \def\t{1.83}
            \def\z{1 + \t * 2}
            \def\u{6 - \t * 3}
            \draw[thick,red] (0,7.5) -- (\z,\u);
            \draw[thick,dotted,red] (\z,\u) -- (7,-3);
            \def\s{0.39}
            \def\v{-2.5 + \s * 9.5}
            \def\w{2 - \s * 2}
            \def\ss{1.1}
            \def\vv{-2.5 + \ss * 9.5}
            \def\ww{2 - \ss * 2}
            \draw[thick,dotted,red] (-2.5,2) -- (\v,\w);
            \draw[thick,red] (\v,\w) -- (\vv,\ww);
            \node at (3,1.5) {$?$};
            \node at (0.5,3) {$\mathcal{R}_1$};
            \node at (2.8,0) {$\mathcal{R}_2$};
            \node at (4.7,2.7) {$\mathcal{R}_3$};
            \draw[<->,thick] (-1,1) node[anchor=east] {$\mathcal{C}_1$}
            -- (1,-1) node[anchor=north] {$\mathcal{C}_2$};
            \def\a{3.8}
            \def\b{-6}
            \draw[<->,thick] (3.8 - \a,-1 - \b) node[anchor=north east] {$\mathcal{C}_1$}
            -- (6.2 - \a,0.6 - \b) node[anchor=south west] {$\mathcal{C}_3$};
            \def\d{0.3}
            \def\e{7}
            \def\f{-1.5}
            \def\k{\e + \d * 2}
            \def\l{\f + \d * 9.5}
            \draw[<->,thick] (\e,\f) node[anchor=north] {$\mathcal{C}_2$}
            -- (\k,\l) node[anchor=south] {$\mathcal{C}_3$};
        \end{tikzpicture}
    \end{center}
\end{frame}
...
\begin{frame}
    ...
    \begin{center}
        \begin{tikzpicture}[scale=0.7]
            ...
            \def\a{0}
            \def\b{0}
            \draw[thick,red] (\a, \b) -- (\s * \x, \s * \y);
            \draw[thick,red] (\a, \b) -- (\t * \z, \t * \u);
            \draw[thick,red] (\a, \b) -- (\k * \v, \k * \w);
            \node at (\a - 1.3, \b + 0.5) {$\mathcal{R}_i$};
            \node at (\a + 0.5, \b - 1.3) {$\mathcal{R}_j$};
            \node at (\a + 1,\b + 1) {$\mathcal{R}_k$};
        \end{tikzpicture}
    \end{center}
\end{frame}
...
\begin{frame}
    ...
    \begin{figure}
        \caption{\only<1>{単連結}\only<2>{単連結ではない}}
        \begin{center}
            %http://www.texample.net/tikz/examples/homotopy/
            %http://tex.stackexchange.com/questions/74168/how-can-i-draw-an-egg-using-tikz
            \begin{tikzpicture}[>=stealth']
                \def\eggheight{2.5}
                \path[fill=red!60]
                plot[domain=-pi:pi,samples=100]
                ({.78*\eggheight *cos(\x/4 r)*sin(\x r)+0.5},{-\eggheight*(cos(\x r))})
                -- cycle;
                \def\out{23}
                \def\in{65}
                \def\return{90}
                ...
                \def\midy{1.5}
                \node (M) at (\midx,\midy) {};
                \node [label=below:$p_0$]  (p0) at (-0.5,-1) {$\bullet$};
                \draw [-] (p0.center) to[in=-\in,out=\out+10]++(0.6 * \midx, 0.7 * \midy)
                to[out=180-\in,in=\return-1] (p0.center);
                \draw [->] (p0.center) to[in=-\in,out=\out]++(M)
                to[out=180-\in,in=\return] (p0.center);
                \pause
                \path [draw=none,fill=white] (0.5,-0.12) circle (0.2);
            \end{tikzpicture}
        \end{center}
    \end{figure}
\end{frame}
...
\begin{frame}
    ...
    \footnote{証明は簡単だが省略する．たとえば海野らを参照．}．
\end{frame}
...
\begin{frame}
    ...
    \footnote{標準正規分布の累積分布関数$\varPhi$の逆関数をプロビット関数と呼ぶ．}．
    ...
    \begin{figure}
        ...
        \begin{center}
            \begin{tikzpicture}
                ...
                xtick={-3,0,3},
                ytick={0,0.5}]
                \addplot[thick] plot{normcdf(x,0,1)} node[pos=0.4](cum){};
    ...
    \begin{figure}
        ...
        \begin{center}
            \begin{tikzpicture}
                ...
                axis line style={->},
                ticks=none]
                \addplot[color=blue,thick] plot (\x, {0.35*exp(-(x*x)/2)/(1 + exp(-20*x-4))});
                \addplot[dashed,color=green,thick] plot (\x, {(1/sqrt(2*pi)) * exp(-(x*x)/2)});
                \draw[dashed] (0,-0.05) -- (0,0.5);
                \node at (0,0) [anchor=north west]{$z_0$};
                \end{axis}
            \end{tikzpicture}
        \end{center}
    \end{figure}
...
\begin{frame}
    ...
    \end{align}
    で与えられる\footnote{$\mathcal{M}$の条件付けを省略した．}．
...
\begin{frame}
    ...
    ヘッセ行列が非退化\footnote{この仮定が満たされていないことが多い．}ならば，
    \begin{align}
